<?php
 ...
 if (isset($_POST['function']) && $_POST['function'] == 'du_building_id') {
    ...
    $axSql_floor= $oDB->Query("SELECT * FROM tbl_location WHERE building_id = '$id' GROUP BY floor_id "); 
    while ($asResult = $axSql_floor->FetchRow(DBI_ASSOC)) {
        ...
        echo '<option>Choose Floor</option>';
        foreach($asFloor as $sFloor) {     
        if($axSql['floor_id']==$sFloor['id']) { 
        echo '<option value="'.$sFloor['id'].'" selected >'.$sFloor['floor_name'].'</option>';
        }else{ 
        echo '<option value="'.$sFloor['id'].'">'.$sFloor['floor_name'].'</option>';
        }     
        } 
 ...
 if (isset($_POST['function']) && $_POST['function'] == 'du_floor_id') {
    ...
    $axSql_room= $oDB->Query("SELECT * FROM tbl_location WHERE floor_id = '$id' GROUP BY room_id "); 
 ...
 if (isset($_POST['function']) && $_POST['function'] == 'du_room_id') {
    ...
    $axSql_rack= $oDB->Query("SELECT * FROM tbl_location WHERE room_id = '$id' GROUP BY rack_name"); 

    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>Register Equipment Form (Non-SAP)</h1>
            </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item active">Register Equipment Form (Non-SAP)</li>
              </ol>
            </div>
          </div>
        </div><!-- /.container-fluid -->
              ...
              <div class="col-md-8">
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input type="input" name="du_asset_code" id="du_asset_code" value="<?php echo isset($axSql['asset']) ? $axSql['asset'] : ''; ?>" class="form-control" placeholder="Asset Code..." >
                    </div>
                  </div>
                  <div class="col-md-4">
                    <div class="form-group">
                ...
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <label>Owner</label>
                      <input type="input" name="du_owner" id="du_owner" value="<?php echo isset($axSql['owner']) ? $axSql['owner'] : ''; ?>" class="form-control" placeholder="Owner..." >
                    </div>
                  </div>
                </div>
 ...
 -->
              ...
              <div class="col-md-4">
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group">
                      <img class="img-fluid pad" src="./img/register-nonSAP.png" alt="Photo">
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="col-md-12">
                ...
                <button type="button" class="btn btn-primary" onclick="validateAndSubmit()" >Save Draft Equipment Data</button>
                  <button type="button" class="btn btn-success"  onclick="validateAndSentSubmit()">Confirm Equipment Data</button>
  ...
  <script>
      async function validateAndSubmit() {
        const assetCode = document.getElementById('du_asset_code').value;
        
        // Basic Validation Example (adjust according to your needs)
        if (!assetCode) {
        ...
        try {
            // Assuming fetch_parent now returns a boolean for validation (true for pass, false for fail)
            const isValid = await fetch_parent(assetCode);
            ...
            if(isValid===null)
            {
              document.getElementById('dataform').submit();
                ...
                // If validation passes, submit the form
                 const tableText = generateTableText(isValid);
                alert(tableText);
    ...
    async function validateAndSentSubmit() {
        const assetCode = document.getElementById('du_asset_code').value;
        ...
        try {
            ...
            if(isValid===null)
            {
              let fromD=document.getElementById('dataform');
              fromD.action = 'register_form_confirm.php?id=<?php echo $sID; ?>';
              fromD.submit();
            }
            else if (isValid.length > 0) {
                // If validation passes, submit the form
    ...
    async function fetch_parent(asset_code) {
        try {
            const response = await fetch('queryData.php?action=queryParent&asset_id=' + asset_code);
            if (response.ok) { // Checks if status is within the range 200-299
                const jsonData = await response.json(); // Parse the JSON from the response
                 return jsonData;
                // gen_player(jsonData,chName,id) // Assuming you have this function defined elsewhere
            } else {
    ...
    function generateTableText(data) {
    // Start with the header
        let tableText = "Asset Code Existing!! Please fill Parent Asset.\n";
         tableText += "Asset Code\tParent Asset\n";
        ...
        data.forEach(item => {
            tableText += `${item.asset_code}\t${item.parent_asset || 'N/A'}\n`;
        });
